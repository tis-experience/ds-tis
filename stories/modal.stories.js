...
function modalMarkup({ size, title, description, id = 'story-modal', customBody = false }) {
  return `<button class="ds-button ds-button--outline" type="button" data-ds-modal-open="${id}"><span class="ds-button__label">Abrir modal ${size}</span></button><div class="ds-modal-overlay" id="${id}" hidden><div class="ds-modal ds-modal--${size}" role="dialog" aria-modal="true" aria-labelledby="${id}-title" aria-describedby="${id}-description"><header class="ds-modal__header"><div class="ds-modal__heading"><h2 class="ds-modal__title" id="${id}-title">${escapeHtml(title)}</h2><p class="ds-modal__description" id="${id}-description">${escapeHtml(description)}</p></div><button class="ds-modal__close" type="button" aria-label="Fechar modal">${icon('x')}</button></header><div class="ds-modal__body">${customBody ? '<div class="ds-field"><label class="ds-field__label" for="modal-email">E-mail</label><div class="ds-input"><input class="ds-input__field" id="modal-email" type="email" placeholder="nome@empresa.com"></div><p class="ds-field__helper">Enviaremos um convite para este endereço.</p></div>' : '<p>Confira os dados antes de aplicar esta atualização reversível.</p>'}</div><footer class="ds-modal__footer"><button class="ds-button ds-button--outline" type="button" data-ds-modal-close><span class="ds-button__label">Cancelar</span></button><button class="ds-button ds-button--brand" type="button"><span class="ds-button__label">Aplicar alterações</span></button></footer></div></div>`;
}
