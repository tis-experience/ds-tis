...
function comboboxMarkup(args, suffix = 'playground') {
  ...
  return fieldMarkup({
    ...
    control: (describedBy) => `<div class="ds-combobox-anchor"><div class="ds-combobox ds-combobox--${size}${stateClasses ? ` ${stateClasses}` : ''}">${showLeadingIcon ? icon('search', 'ds-combobox__icon') : ''}<input class="ds-combobox__input" id="${id}" type="text" role="combobox" aria-expanded="${isOpen}" aria-controls="${id}-list" aria-autocomplete="list" placeholder="${escapeHtml(placeholder)}" value="${escapeHtml(inputValue)}"${describedBy ? ` aria-describedby="${describedBy}"` : ''}${required ? ' aria-required="true"' : ''}${isError ? ' aria-invalid="true"' : ''}${isDisabled ? ' disabled' : ''}${isReadonly ? ' readonly' : ''}>${showClearButton && !isDisabled && !isReadonly ? `<button class="ds-combobox__clear" type="button" aria-label="Limpar seleção"${hasValue ? '' : ' hidden'}>${icon('x')}</button>` : ''}${icon('chevron-down', 'ds-combobox__chevron')}</div><ul class="ds-combobox__listbox" id="${id}-list" role="listbox" aria-label="Países"${isOpen ? '' : ' hidden'}><li class="ds-combobox__option" role="option">Argentina</li><li class="ds-combobox__option" role="option" aria-selected="${hasValue}">Brasil</li><li class="ds-combobox__option" role="option">Chile</li><li class="ds-combobox__option" role="option">Portugal</li><li class="ds-combobox__option" role="option" aria-disabled="true">Indisponível</li></ul></div>`,
  });
}
