...
O resultado público é composto por **quatro saídas coexistentes**, nunca por um
provider vencedor ou por uma implementação que some as quatro: (1) HTML/CSS +
JavaScript agnóstico, (2) Ark UI + Zag, (3) React + shadcn + Base UI e (4) Angular
nativo. shadcn fornece estrutura de composição e distribuição de source para a
terceira saída; Base UI fornece seus primitives comportamentais. Angular usa HTML
nativo, Angular Aria e Angular CDK conforme o padrão, sem Angular Material visual.
Cada saída mantém source, dependências, instalação, API e testes próprios, e a
documentação deve permitir que a pessoa escolha qual delas visualizar e utilizar.
As quatro compartilham tokens, linguagem visual, requisitos de acessibilidade e
contrato conceitual, sem imports cruzados nem eliminação de uma saída em favor de
outra. Ver ADR-022 e ADR-023.
