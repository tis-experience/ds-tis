...
3. Dump tipográfico obrigatório em `evidence/`: `textStyleId` + binds de cada classe de TEXT do modelo (não basta ler o layout).
4. Se houver Content Slot na spec: dump do slot em Modal/Card (defaults, vazio no master, ordem Show→Slot).
5. Capturar screenshot do alvo atual quando existir e screenshots dos modelos.
6. Registrar o modelo escolhido na spec ou no checkpoint.
7. Tratar o modelo como contrato operacional, não como inspiração visual vaga.
8. Preferir **clonar** o root da página modelo e adaptar conteúdo a recriar textos com Inter/hex.

Regras mínimas:

- Textos de documentação usam altura automática (`textAutoResize` adequado / height hug), não altura fixa criada por script.
- Textos usam o mesmo Text Style e variables de cor/tipografia do modelo; `fontName`/`fontSize`/`fills` crus são regressão.
...
- Content Slot segue Modal/Card: `createSlot()` no component, `Show Content Slot=false` no set, slot vazio no master, exemplos só em instâncias na página.
...
- textos documentais ou de set sem `textStyleId`/binds quando o modelo equivalente os usa;
...
- Content Slot com seed no master, chrome próprio ou default `true` sem exceção;
