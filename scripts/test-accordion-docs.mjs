 ...
 * - The public runtime synchronizes aria-expanded, hidden, and data-state.
 */
...
const js = fs.readFileSync(path.join(ROOT, 'js', 'accordion.js'), 'utf-8');
const errors = [];
...
expect(docs.includes('ds-tis/accordion') || docs.includes('../js/accordion.js'), 'Accordion docs must point to the public runtime module.');
expect(docs.includes('initAccordions'), 'Accordion docs must mention initAccordions.');

expect(js.includes('export function initAccordions'), 'Accordion public module must export initAccordions.');
expect(js.includes('export function destroyAccordions'), 'Accordion public module must export destroyAccordions.');
...
expect(js.includes('ds-accordion-open') && js.includes('ds-accordion-close'), 'Accordion must emit open/close events.');
