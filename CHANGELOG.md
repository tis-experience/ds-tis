...
- **Catálogo explicita as quatro implementações de cada componente.** Os 26 cards
  ...
  enquanto somente implementações funcionais oferecem navegação.
...
- **Previews funcionais aguardam a renderização real do Storybook.** O portal
  ...
  Os testes de consumer aguardam também as transições assíncronas de foco de
  Accordion, Tabs, Menu, Popover e Tooltip sem reduzir as asserções de teclado.
...
- **Anatomia e exemplos do Modal preservam a geometria HTML/CSS no portal.**
  ...
  alinhamento e alturas públicos, e os sete marcadores da anatomia não são
  recortados. O teste de browser passa a bloquear a regressão nas quatro rotas.
...
- **Tabelas de referência do portal vNext ocupam toda a largura disponível.**
  ...
  foco acessível e ausência de overflow global em desktop, 390px e 320px.
- **Playground e anatomia do Popover consistentes nas quatro saídas.** Web,
  Ark/Zag, React e Angular passam a abrir cenários equivalentes executados pelo
  Storybook e runtime próprios de cada tecnologia, sem reutilizar o exemplo Web.
  Os actions usam dois Buttons alinhados, as quatro setas preservam contorno e
  preenchimento sem divisória, e a anatomia compacta identifica Trigger, Panel,
  Header, Body, Actions e Arrow com seis marcadores numerados. Rebuilds parciais
  do portal preservam também o Storybook Angular, evitando previews em 404. Os
  exemplos com Content Slot recebem altura própria para exibir campo e actions
...
- **Action Menu passa a integrar a saída Angular nativa em beta de workspace.**
  ...
  relações ARIA. Storybook, harness, consumer instalado, responsividade, dark
  mode, Axe e bundle validam a quarta implementação. O label de grupo do Menu
  compartilhado também passa a preservar contraste AA sobre a superfície dark,
  sem alterar Figma ou tokens.
...
- **Toggle passa a integrar a saída Angular nativa em beta de workspace.** O
  ...
  `ControlValueAccessor` ao Angular Forms. Storybook, harness, consumer instalado,
  responsividade, dark mode, Axe e bundle validam a quarta implementação sem
  alterar Figma, tokens ou as outras três saídas.
- **Radio passa a integrar a saída Angular nativa em beta de workspace.** Os
  entrypoints `TisRadioGroup` e `TisRadioOption` usam fieldset, legend e inputs
  radio nativos, preservam seleção exclusiva, setas, disabled, required e
  invalid e integram `ControlValueAccessor` ao Angular Forms. Storybook, harness,
  consumer instalado, responsividade, dark mode, Axe e bundle validam a quarta
  implementação sem alterar Figma, tokens ou as outras três saídas.
- **Checkbox passa a integrar a saída Angular nativa em beta de workspace.** O
  entrypoint `@tis/angular/checkbox` usa checkbox nativo, preserva checked,
  indeterminate, disabled, required e invalid e integra `ControlValueAccessor`
  e validação required do Angular Forms. Storybook, harness, consumer instalado,
  responsividade, dark mode, Axe e bundle passam a validar a quarta implementação
  sem alterar Figma, tokens ou as outras três saídas.
- **Modal passa a integrar a saída Angular nativa em beta de workspace.** O novo
  entrypoint `@tis/angular/modal` preserva a anatomia e os tokens existentes com
  CDK Overlay/Portal/A11y, estado controlado, focus trap, Escape, backdrop,
  retorno de foco e tamanhos `sm`, `md` e `lg`. Storybook, consumer instalado,
  documentação, harness, responsividade, dark mode, Axe e bundle passam a
  validar a quarta implementação sem alterar Figma ou as outras três saídas.
- **Quarta saída tecnológica Angular nativa em beta de workspace.** Button,
  Accordion e Popover recebem entrypoints tree-shakable em `@tis/angular`,
  Storybook próprio, consumer real e rotas PT-BR/EN no seletor documental. A
  biblioteca usa HTML nativo, Angular Aria e CDK Overlay/Portal/A11y, consome o
  CSS existente e preserva Figma, tokens e as saídas Web, Ark/Zag e React.
- **Button conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  A saída Ark usa `ark.button` da Ark Factory e mantém semântica e teclado
  nativos, sem máquina Zag desnecessária. Variantes, tamanhos, disabled,
  loading, formulário, foco, visual, acessibilidade e bundle passam a ser
  validados junto às saídas Web e React/Base UI independentes.
- **Toggle conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  Ark UI/Zag recebe adapter independente com estados on/off, tamanhos, disabled,
  foco, teclado e hidden input de formulário. Web nativo e a recipe React/Base
  UI permanecem independentes e compartilham somente tokens, linguagem visual
  e contrato conceitual; browser, Axe e bundle passam a bloquear regressões.
- **Radio conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  Ark UI/Zag recebe adapter independente com seleção exclusiva, setas, Space,
  disabled, invalid, foco e hidden inputs de formulário. Web nativo e a recipe
  React/Base UI permanecem independentes e compartilham somente tokens, visual
  e contrato conceitual; browser, Axe e bundle passam a bloquear regressões.
- **Checkbox conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  Ark UI/Zag recebe adapter independente com checked, mixed, disabled, invalid,
  focus e hidden input de formulário. Web nativo e a recipe React/Base UI
  permanecem independentes e compartilham apenas tokens, visual e contrato
  conceitual; browser, Axe e bundle passam a bloquear regressões.
- **Toast conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  Ark UI/Zag recebe adapter independente e o registry React recebe uma recipe
  shadcn baseada em Base UI. As saídas preservam tipos e estilos, fila com
  limite, prioridade acessível, timer pausável, action, dismiss e responsividade;
  browser, consumidor instalado, Axe e bundle passam a bloquear regressões. O
  catálogo React passa a ter vinte e dois componentes beta.
- **Tabs conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  Ark UI/Zag recebe adapter independente e o registry React recebe uma recipe
  shadcn baseada em Base UI. As duas saídas preservam anatomia, tokens,
  ativação automática, roving tabindex, setas, Home/End, disabled ignorado e
  relações entre tabs e painéis. Ark e React impedem quebra de linha nos labels
...
- **Tooltip conclui a paridade local nas três saídas sem alterar o contrato Web/Figma.**
  ...
  comportamento do Tooltip existente. As saídas preservam hover/focus,
  `aria-describedby`, conteúdo hoverable, Escape e foco no trigger; browser,
  consumidor instalado, Axe e orçamentos de bundle passam a bloquear regressões.
  O catálogo React passa a ter vinte componentes beta.
- **Menu conclui a paridade local nas três saídas sem alterar o contrato Web.**
  O Action Menu HTML/CSS/JavaScript e o componente no Figma permanecem
  canônicos; Ark UI/Zag recebe um adapter isolado e o registry React recebe uma
  recipe shadcn baseada em Base UI. Os adapters cobrem comandos, checkbox,
  radio, disabled, destructive, grupos e separadores, preservando alinhamento,
  foco composto, typeahead, Escape e retorno ao trigger. Portal, Storybook,
  consumidor instalado, Axe e orçamentos de bundle passam a validar as três
  saídas, e o catálogo React passa a ter dezenove componentes beta.
- **Select conclui a paridade local nas três saídas sem alterar o Web nativo.**
  O contrato HTML/CSS continua baseado em `<select>`; Ark UI/Zag recebe adapter
  isolado e o registry React recebe uma recipe shadcn baseada em Base UI. As
  três rotas documentam placeholder, opções desabilitadas, typeahead e valor de
  formulário. A validação corrigiu uma colisão de namespace entre providers,
  confirmou largura do popup, teclado, foco, Axe, consumidor instalado e os
  limites de bundle. O catálogo React passa a ter dezoito componentes beta.
- **Combobox conclui a paridade local nas três saídas.** A implementação
  HTML/CSS/JS preserva a API pública e corrige clear, opções desabilitadas e a
  ocultação real dos itens filtrados; Ark UI/Zag recebe adapter e story
  independentes; e o registry React recebe a recipe shadcn baseada somente em
  Base UI. O portal bilíngue expõe as três implementações com conteúdo
  compartilhado, seleção explícita de tecnologia e testes de teclado, foco,
  Axe, consumidor instalado e bundle, sem alterar tokens ou Figma. O catálogo
  React passa a ter dezessete componentes beta.
- **Modal Ark/Zag passa a ter adapter público de workspace.** O subpath
  `@tis/react/ark/modal`, a story isolada e as rotas PT-BR/EN preservam a
  anatomia, os três tamanhos e os tokens do Modal TIS, com foco, teclado,
  portal, bloqueio de scroll e retorno de foco mantidos por Ark UI/Zag. O
  contrato cobre apenas Dialog modal comum e reserva confirmações críticas ou
  destrutivas para um futuro Alert Dialog, sem alterar o Figma, o CSS ou o
  runtime HTML/JavaScript estável. A saída React/Base UI recebe o mesmo exemplo
  reversível, tamanho `md` padrão e semântica `aria-modal` explícita. O adapter
  React também passa a manter largura border-box e close icônico totalmente
  contido na superfície, sem depender de texto auxiliar visualmente oculto.
- **Accordion Ark/Zag deixa de ser apenas um spike interno.** O adapter aprovado
  passa a ter entrypoint de workspace, story pública e rota selecionável no
  portal PT-BR/EN. A saída preserva a anatomia, o visual e os tokens do Accordion
  estável, com estado e teclado fornecidos por Ark UI/Zag, sem importar Base UI,
  shadcn ou alterar a implementação HTML/CSS/JS.
- **Popover inaugura as três saídas tecnológicas selecionáveis.** A implementação
  HTML/CSS/JS permanece estável e agnóstica; um adapter React independente usa
  Ark UI com Zag transitivo; e o registry React recebe um source shadcn baseado
  somente em Base UI. O portal bilíngue permite alternar entre as três rotas sem
  fundir providers, e consumers, browser, Axe e bundle passam a validar cada
  saída. Naquele incremento, o catálogo React cresceu para dezesseis componentes
  beta. Os adapters passam também a diferenciar o close icônico do cabeçalho e a
  ação textual do rodapé, preservando conteúdo, estilo e nomes acessíveis em
  desktop e mobile, sem escrita no Figma ou no core Web.
- **Tronco público de consumo React para Devs e agents IA.** O source registry
  ...
  `/registry/v1`, com namespace `@tis`, manifesto compacto e quinze componentes
...
- **Onda de apresentação do registry React.** Alert, Badge, Card,
  Divider/Separator, Skeleton e Spinner entram no catálogo beta por source,
  reutilizando a anatomia e as classes públicas do DS sem provider
  comportamental desnecessário. Stories e fixture de consumo cobrem composição,
  semântica nativa, estados de loading, responsividade e Axe; o catálogo legível
  por Devs e agents IA passa a expor quinze componentes validados.
...
### Alterado
- **Snapshot Figma passa a validar aliases com uma segunda fonte independente.**
  O exporter `0.2.1` audita a mesma representação serializada que entrega ao
  repositório; `verify:figma-structure` recalcula invariantes de Variables sem
  confiar no diagnóstico embutido, mas continua bloqueando problemas de nodes,
  páginas e truncamentos que possam ocultá-los. A evidência de release preserva
  a contagem informada pelo exporter e registra separadamente o resultado
  canônico verificado pelo repo.
- **Portal vNext reduz carga e repetição sem alterar o contrato das três saídas.**
  O catálogo troca 78 caixas de disponibilidade por uma matriz textual compacta,
  recebe descrições específicas para todos os componentes e remove um callout
  redundante. Previews Storybook sincronizam o tema e carregam imediatamente, sem
  depender de scroll; referências de design deixam de ficar ocultas; e o catálogo
  mostra somente implementações utilizáveis, mantendo trabalho planejado no
  roadmap. A navegação móvel troca tabs sem espaço por um select nativo, e o
  Popover Ark/Zag expõe `placement` no Root com cobertura executável para top,
  right, bottom e left. Os playgrounds comparáveis de Accordion e Popover passam
  a preservar conteúdo, estado inicial, ícones, largura e tamanho do trigger da
  referência HTML/CSS/JS, com regressões no portal e no consumer instalado.
- **Portal vNext passa a ter um catálogo canônico independente de tecnologia.**
  Os 26 componentes aparecem em um único ponto de entrada, com disponibilidade
  real e links próprios para HTML/CSS/JS, Ark/Zag e React/shadcn/Base UI. O
  breadcrumb deixa de apontar para o catálogo React; a troca de implementação
  vira um controle único e rotulado; previews sincronizam o tema antes da
  primeira carga, usam alturas proporcionais e estados de loading acessíveis; e
  a navegação móvel substitui tabs cortadas por um select nativo. O renderer não
  exige mais uma implementação React para documentar Web ou Ark, snippets
  passam a ter scroll local e os testes de browser aguardam o reflow real.
- **Accordion, Button e Popover passam a usar um renderer documental Astro único.**
  As referências estáveis HTML/CSS/JS continuam canônicas e fornecem uma única
  fonte bilíngue para anatomia, exemplos, Figma, tokens, uso e acessibilidade. O
  portal reorganiza o conteúdo no padrão editorial aprovado e oferece três rotas
  por componente, preservando preview, distribuição, source e testes próprios de
  Web, Ark/Zag e React/shadcn/Base UI. Saídas planejadas permanecem explícitas e
  não exibem instalação, código, preview ou evidência falsos.
- **Documentação do Popover recupera o padrão editorial completo do Button.**
  As três saídas passam a compartilhar breadcrumb, descrição, disponibilidade e
  as quatro visões editoriais, mantendo preview, distribuição e API próprios.
  HTML/CSS/JS volta a exibir o Playground nativo do Storybook estável; anatomia,
  exemplos, placements, tokens, uso e teclado são reaproveitados da referência
  canônica bilíngue, sem fundir Web, Ark/Zag e React/shadcn/Base UI.
- **Intake upstream passa a ter orquestração e gates verificáveis.** Um processo
  canônico, manifesto por run e validador automatizado preservam Figma e o
  contrato compartilhado como agnósticos e bloqueiam mistura entre as saídas. A
  ADR-022 formaliza HTML/CSS/JS, Ark/Zag e React/shadcn/Base UI como três saídas
  coexistentes, escolhidas pelo consumidor na documentação e nunca fundidas ou
  submetidas a uma seleção de provider vencedor. O Popover abre a primeira run
  formal de paridade sem autorizar ainda Figma, adapters ou release.
- **Arquitetura React volta a distinguir distribuição e comportamento.** A
  ADR-022, o portal bilíngue, o plano operacional, o registry e os contratos
  machine-readable passam a declarar shadcn como estrutura/composição e canal de
  source da saída React, com Base UI como seu provider comportamental. Ark/Zag e
  HTML/CSS/JS mantêm implementações, distribuição, testes e release próprios. O
  primeiro gate mede a paridade do Accordion nas três saídas sem remover nenhuma.
- **Storybook React reorganizado como catálogo público por componente.** A
  superfície vNext deixa de misturar ondas, formulários, providers e vários
  componentes na mesma story: os quinze componentes beta passam a viver em
  cinco categorias semânticas, cada um com Docs funcional, Playground com
  Controls e ao menos dois exemplos isolados. As tabelas de API passam a expor
  tipo, valor padrão e descrição; a entrada Overview orienta a navegação e
  comparações internas de Ark/Zag ficam fora do build público. Gates estáticos
  e de browser protegem os 16 documentos, 50 stories, links do portal,
  comportamento Base UI, 320/390px, light/dark, Axe e densidade dos exemplos em
  Docs.
- **Portal React consolidado em um único catálogo operacional.** A navegação
  deixa de espalhar componentes entre tecnologia, pilotos, forms, apresentação
  e uma tabela paralela de registry. Os quinze componentes beta passam a ser
  agrupados pelo problema que resolvem e usam o mesmo template bilíngue de
  Design, Uso, Implementação e Acessibilidade — inclusive Button. Cada página
  reúne preview funcional sincronizado com o tema do portal, anatomia, estados,
  exemplo copiável, instalação e evidência; provider e source ficam recolhidos
  como arquitetura técnica. Web CSS, React e agents IA tornam-se guias de
  integração separados, enquanto API, registry e metadados preservam a fonte
  estruturada para Devs e agents IA.
...
### Corrigido
- **Exemplos estáticos de Modal recuperam a apresentação TIS no portal vNext.**
  O adaptador de conteúdo do Astro passa a carregar as folhas públicas de Modal,
  Button, Form Field e Input usadas pelas composições, evitando superfícies
  transparentes e controles nativos sem estilo nas rotas Web, Ark/Zag e React.
...
### Corrigido
- **Theme Playground mantém contraste AA ao trocar o modo global.** Alterações externas em `data-mode` agora recalculam os overrides inline do theme engine; o scan HTTP estrito do playground em light/dark integra `npm test` e bloqueia qualquer nova ocorrência `critical`/`serious`.
- **Tooltip conclui o gate App-ready.** O runtime passa a aceitar o próprio component root sem envenenar markup incompleto, gerar role/ID/`aria-describedby` válidos, cancelar timers e callback de foco no destroy e publicar eventos com `detail` estável. Lifecycle prova foco, blur, Escape, conteúdo hoverable, hydration e re-init; o consumer smoke valida interação e axe aberto no tarball antes da promoção individual.
- **Tabs conclui o gate App-ready.** O runtime passa a aceitar o próprio tablist root sem envenenar markup incompleto, normaliza tabs em formulários para `type="button"`, assegura entrada de foco no tabpanel e publica `detail` estável. Lifecycle prova root/hydration, teclado, disabled, ARIA, eventos, destroy e re-init; o consumer smoke valida seleção sem submit e axe dos painéis no tarball antes da promoção individual.
- **Action Menu conclui o gate App-ready.** O runtime passa a suportar `menuitemradio` e `menuitemcheckbox`, impedir ativação de itens `aria-disabled`, manter typeahead com cleanup e aguardar a visibilidade real do popup antes de mover foco. Lifecycle prova root/hydration, teclado, roles, eventos e destroy; o consumer smoke valida interação e axe aberto no tarball antes da promoção individual.
- **Modal conclui o gate App-ready.** O runtime passa a aceitar o próprio overlay root sem envenenar markup incompleto, inertiza somente os irmãos fora do caminho do diálogo e restaura estados `inert` preexistentes. Lifecycle prova focus trap, retorno de foco, ARIA, backdrop, eventos, idempotência e cleanup; o consumer smoke valida o tarball com axe no Modal aberto antes da promoção individual.
- **Combobox conclui o gate App-ready.** O runtime passa a aceitar document, container e o próprio root, mantém markup incompleto recuperável, sincroniza `aria-activedescendant` com a opção ativa sem mover o foco DOM, preserva foco no Escape e publica `detail` estável no evento. Lifecycle prova teclado, ARIA, idempotência e cleanup; o consumer smoke valida o tarball com axe nos estados fechado/aberto antes da promoção individual.
- **Accordion conclui o gate App-ready.** `initAccordions` passa a aceitar o próprio component root e não envenena markup incompleto antes de hydration. Lifecycle prova root/container/component init, late hydration, idempotência, teclado, foco circular, ARIA, eventos e cleanup; o consumer smoke instala o tarball e valida interação, zero erros de browser e axe nos estados fechado/aberto antes da promoção individual.
...
- **Branding da documentação.** Topbar passa a exibir a marca TIS no lugar do ícone "DS" em todas as páginas HTML; home atualizada com o mesmo nome.
...
- **Componente Combobox.** Sincroniza 16 tokens `component.combobox.*` do Figma, `css/components/combobox.css`, `docs/combobox.html`, listbox/option com tokens locais + geometria `menu/item/*`, field compartilhado (ADR-019), ARIA de combobox/listbox/option, baselines visuais e links em Select/Menu.
...
- **Theme Playground: combobox de fontes alinhado ao componente Combobox do DS.** Campos de tipografia passam de `input` + `datalist` (chevron nativo do browser) para `ds-combobox` com chevron Lucide, listbox popup e suporte a digitação livre.
- **Combobox: anatomia de popup + módulo JS compartilhado.** Wrapper `.ds-combobox-anchor` posiciona o listbox abaixo do field; `[hidden]` e `.ds-combobox--open` controlam visibilidade e chevron; `js/combobox.js` implementa abrir/fechar exclusivo, blur, filtro e teclado (usado no Theme Playground).
- **Form controls: radius estável no focus.** Combobox deixou de trocar `border-radius` para `radius-lg` em `:focus-within`; Input, Select e Textarea fixam `border-radius: var(--ds-field-radius)` no focus para não “arredondar” visualmente em relação ao estado default.
- **Theme Playground: exportação vazia e UI de export.** `renderContrast` quebrava em overlays `rgba` e interrompia `renderExports`; contraste agora compõe overlay sobre superfície. Seção Export usa layout próprio (radius fixo, sem conflito com tabs de docs) e explica CSS / JSON / Link.
- **Theme Playground: preview de Card sem variante.** Card passa a usar `ds-card--default` com anatomia completa (`__container`, `__header`, `__footer`); a borda só existe nas variantes do componente, não no `.ds-card` base.
- **Overlays toned desacoplados da paleta neutra (base para theme playground).** `foundation.color.overlay.blue-600.{12,20,28}` e `overlay.blue-400.{15,25,32}` renomeados para `overlay.brand-600.*` e `overlay.brand-400.*`, e `semantic.toned.background.*` (light/dark) passam a referenciá-los. Isso faz os botões/badges `toned` derivarem da paleta brand customizável em vez de um azul hardcoded — pré-requisito para personalização global de marca em tempo real. Corrige também o dark, que referenciava por engano os overlays do light (mesmo alpha); agora usa `brand-400.{15,25,32}` e os overlays antes órfãos passam a ter uso. Valores rgba inalterados (brand default = blue), sem regressão visual. **Figma:** variables `color/overlay/blue-*` renomeadas para `color/overlay/brand-*`; aliases `toned/background/*` no modo Dark atualizados para `brand-400`. Ver nota de atualização na ADR-007.
- **Estabilização Accordion, Combobox e Pagination (run `2026-07-06-stabilize-recent-components`).** Auditoria paralela Figma↔repo com snapshot fresco; gates `test-combobox-docs` e `test-pagination-docs`; documentação alinhada à API Figma e tokens canônicos.
- **Documentação Pagination.** Corrige paths `component.pagination.page.*` → `item.*`, expande mapeamento de tokens (29 paths) e adiciona seção API no Figma (`Size`, `Page Number`).
- **Documentação Combobox.** Adiciona API no Figma, exemplo Read-only, classes `__icon`/`__clear`/`--readonly`; `focus-visible` em `.ds-combobox__option`; `max-block-size` do listbox via `calc(var(--ds-dimension-128) * 2)`.
- **Documentação Accordion.** Adiciona seção Boas práticas (`ds-dodont`) e classe `ds-accordion__item--disabled` na tabela pública.
- **Docs de overlay brand e milestones.** `foundations-colors.html` e `control-sizing.html` atualizam referências para `--ds-overlay-brand-*`; inventário passa a listar Table, Toast e Popover como próximos componentes.
...
- **Pendências de maturidade e milestones removem itens stale.** `process-versioning` passa a marcar Form Field CSS-only, gates locais de snapshot, registry sem warnings, visual regression local e a11y tests como entregues; `component-inventory` milestones atualizados para próximos componentes reais (Table, Toast, Popover).
